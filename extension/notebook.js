...
async function deleteDeck() {
  ...
  const s = await getStore();
  const now = Date.now();
  // chuyển các từ thuộc sổ này về chưa phân loại
...
// ---- Âm Hán Việt từ dữ liệu offline ----
...
function isDue(it, now) {
  ...
  const srs = it.srs;
  if (!srs || !srs.due) return true;        // từ mới: đến hạn ngay
  return srs.due <= now;
}
...
async function setFavStudy(it, val) {
  ...
  s.nb[it.key] = ne;
  await setNotebook(s.nb);
  it.fav = next;                 // cập nhật thẻ đang học để nút đổi trạng thái ngay
  renderStudyFav(it);
  syncSoon();
}
...
let lastDeleted = null;   // { key, entry } để hoàn tác
...
async function deleteCurrentCard() {
  ...
  // bỏ hết bản sao của từ này khỏi hàng đợi (khi "Quên" nó bị xếp lại cuối hàng)
...
document.getElementById("stSpk").addEventListener("click", () => { const it = session.queue[0]; if (it) speakJa(it.word); });
...
function draw() {
  ...
  for (const it of rows) {
    ...
    if (hvStr) { const hvEl = document.createElement("div"); hvEl.className = "hv"; hvEl.textContent = "Hán Việt: " + hvStr; main.appendChild(hvEl); }
    if (it.means && it.means.length) {
...
function exportAnki() {
  ...
  download("njdict-anki-" + fileTag() + ".tsv", lines.join("\n"), "text/tab-separated-values;charset=utf-8");
}
function csvCell(s) { s = String(s == null ? "" : s); return /[",\n]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s; }
function exportCsv() {
  const list = currentActiveSet();
  if (!list.length) return;
  const header = ["Từ", "Cách đọc", "Nghĩa", "Sổ", "Hướng", "Ngày lưu"];
  ...
  download("njdict-sotay-" + fileTag() + ".csv", csv, "text/csv;charset=utf-8");
}

// ---- Sao lưu / Nạp (.json) ----
async function backupJson() {
  const s = await getStore();
  download("njdict-sotay-backup.json", JSON.stringify({ notebook: s.nb, decks: s.decks }), "application/json;charset=utf-8");
}
...
async function restoreJson(file) {
  try {
    ...
    const s = await getStore();
    // hỗ trợ cả file cũ (chỉ là object notebook) lẫn file mới {notebook,decks}
...
async function saveSettings() {
  const S = {
    ...
    maxLen: Math.max(5, Math.min(200, parseInt(document.getElementById("setLen").value, 10) || 30)),
