/* Vá thư mục android/ sau khi `npx cap add android` hoặc `npx cap sync android`.
   ...
   2. Thêm 2 activity-alias vào AndroidManifest.xml:
        - "Tra bằng NJDict"  (ACTION_PROCESS_TEXT) — menu bôi đen.
        - "Lưu vào NJDict"   (ACTION_SEND)        — hiện trong bảng Chia sẻ, nhận cả link.
   3. Đổi tên hiển thị app thành NJDict trong strings.xml. */
const fs = require("fs");
const path = require("path");

const APP_ID_PATH = path.join("android", "app", "src", "main", "java", "com", "nhien", "tramazii");
...
`;
  ...
  console.log("✓ Đã thêm mục 'Tra bằng NJDict' vào menu bôi đen (AndroidManifest)");
} else {
...
`;
  ...
  console.log("✓ Đã thêm mục 'Lưu vào NJDict' vào bảng Chia sẻ (AndroidManifest)");
} else {
...
if (fs.existsSync(STRINGS)) {
  ...
  st = st.replace(/(<string name="app_name">)[^<]*(<\/string>)/, "$1NJDict$2");
  st = st.replace(/(<string name="title_activity_main">)[^<]*(<\/string>)/, "$1NJDict$2");
  fs.writeFileSync(STRINGS, st);
  console.log("✓ Đã đổi tên hiển thị thành NJDict (strings.xml)");
}
